        ...
        | (
            ...
            let txs: Vec<_> = query
                ...
                .history(
                    ...
                    Some(CHAIN_TXS_PER_PAGE),
                    max_txs,
                )
                .map(|res| {
                    let (tx, blockid) = res?; // question mark to propagate error
                    Ok((tx, Some(blockid)))
                })
                .collect::<Result<Vec<(bitcoin::Transaction, Option<BlockId>)>, errors::Error>>()?;
...
fn parse_scripthash(scripthash: &str) -> Result<FullHash, HttpError> {
    let mut bytes: FullHash = FromHex::from_hex(scripthash)?;
    if bytes.len() != 32 {
